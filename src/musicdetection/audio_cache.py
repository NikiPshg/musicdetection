...
def measure_audio_length(file_path: str) -> tuple:
    """
    Measures the duration of a single audio file.

    Args:
        file_path (str): Path to the audio file.

    Returns:
        tuple: A tuple containing the file path and its duration in seconds.
    """
    try:
...
def create_audio_length_cache(
    ...
    """
    Creates or loads a cache of audio file lengths to speed up batching.

    Args:
        file_paths (List[str]): A list of paths to the audio files.
        cache_file (str, optional): Path to save/load the cache file. Defaults to None.
        num_workers (int, optional): Number of CPU cores to use. Defaults to available cores.
        force_rebuild (bool, optional): If True, rebuilds the cache even if it exists. Defaults to False.

    Returns:
        Dict[str, float]: A dictionary mapping file paths to their durations.
    """
